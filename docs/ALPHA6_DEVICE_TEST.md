...
目标：验证 user10 CE 解锁 gate，并定位切换到 user10 后是否真的出现可输入密码的前台界面。
...
- 带密码尝试会先执行 `cmd lock_settings verify --old <已脱敏> --user 10`；如果仍未解锁，会切换到 user10，显式拉起 user10 Home，唤醒/滑开锁屏，依次尝试 KEYCODE_0-9、两组屏幕 PIN 键盘坐标点击、`input text`。日志只记录结果分类、Activity/Window/Power 摘要、坐标网格和密码长度，不记录明文。
...
- 如果失败，请重点回传这些行：`CURRENT_USER_AFTER_SWITCH`、`BRING_CLONE_HOME_OUTPUT`、`BEFORE_INPUT_*`、`AFTER_PREPARE_*`、`AFTER_DIGIT_KEYEVENTS_*`、`AFTER_PIN_PAD_TAPS_*`、`AFTER_INPUT_TEXT_*`。它们能判断输入到底打到了锁屏、Launcher、空焦点，还是被系统拦截。
