# v0.1.0-alpha.3 真机测试说明

目标：验证 user10 CE 解锁 gate、主动备份/切换不再误报成功，并导出恢复一致性审计包。

## 1. 安装与授权

1. 安装 `v0.1.0-alpha.3` APK 到主系统 user0。
...
- 如需实验性自动解锁，先到「设置」填写分身锁屏 PIN/密码，再在「诊断」点击「带密码尝试解锁」。
- 带密码尝试会先执行 `cmd lock_settings verify --old <已脱敏> --user 10`，再切换到 user10 锁屏界面输入；日志只记录结果分类和密码长度，不记录明文。
