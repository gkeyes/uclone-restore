...
class TaskCoordinator(
    ...
    private val transactionRecovery: TransactionRecoveryRepository = TransactionRecoveryRepository.ready(),
) {
    ...
    fun accept(
        ...
        transactionRecovery.blockingMessage(type)?.let {
            return@synchronized TaskSubmissionResult.RecoveryRequired(it)
        }
        val current = active
        if (current != null) {
    ...
    fun tryRunWhileIdle(block: () -> Unit): Boolean = synchronized(monitor) {
...
sealed interface TaskSubmissionResult {
    data class Accepted(val record: TaskRecord) : TaskSubmissionResult
    data class AlreadyRunning(val record: TaskRecord) : TaskSubmissionResult
    data class AlreadyCompleted(val record: TaskRecord) : TaskSubmissionResult
    data class Busy(val record: TaskRecord) : TaskSubmissionResult
    data class RecoveryRequired(val message: String) : TaskSubmissionResult
}
